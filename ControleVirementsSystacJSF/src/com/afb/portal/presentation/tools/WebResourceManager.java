...
public class WebResourceManager extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("----doGet---ViewHelper.userId------"+ViewHelper.userId);
		
		Long uid = ViewHelper.userId == null ? null : Long.valueOf(ViewHelper.userId);
		
		// Recuperation de l'id de l'utilisateur connecte
		if(uid == null) uid = request.getParameter("uid") == null ? null : Long.valueOf(request.getParameter("uid"));
		...
		if(uid != null){
			...
			if ( ViewHelper.creditManager != null ){
				// Recherche de l'utilisateur connecte
				User user = ViewHelper.creditManager.findByPrimaryKey(User.class, uid, null);
				// Si l'utilisateur a ete retrouve
				if( user != null ){
					
					String libuserId = "Bienvenue "+user.getName()+" à Convention Credit Scolaire";									
    	...
    	String resourcePath = "file:////" + ViewHelper.ROOT_DATAS_DIR.replaceAll("\\\\", "/") + requestURI;
